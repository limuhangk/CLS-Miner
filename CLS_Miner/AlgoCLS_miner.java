package ca.pfv.spmf.algorithms.frequentpatterns.CLS_Miner;
...
//                tid++; // increase tid number for next transaction
     ...
     * @throws IOException
     */
    private void CLS_Miner(boolean firstTime, int[] closedSet, UtilityList closedSetUL,
        ...
        for (UtilityList iUL : postset) {
            ...
            if (isPassingHUIPruning(newgen_TIDs)) {
                ...
                // L5:  if newgen is not a duplicate #use improved isSubsumedCheck //use precheck method
...
//					// L7 : PostsetNew = emptyset
                    ...
                    for (UtilityList jUL : postset) {
                        ...
                        //L9 if(con(Px,y)<minutil) continue; con(X,y) is the Conjunction upper-bound utility LBP 使用EUCS结构时，剪枝效果不太明显。。。
                        if (useLBP && calculate_Con(newgen_TIDs, jUL) < minUtility) {
                            continue;
                        }
                        //L12 if(∃i∈px such that EUCS(i,y)<minutil) continue; Generalized EUCP property ---first strategy
                        if (useChain_EUCP) {
...
//                        candidateCount++;
                        ...
                        //L15 if(y∈Cov(x) OR (preCheckContain(Px,y) AND TidSet(Px)⊆TidSet(y))) Coverage concept ---second strategy
                        if (usePreCheck || useCoverage) {
                            if (useCoverage && ifBelongToCov(iUL.item, jUL.item) || usePreCheck && (preCheckContain(jUL, newgen_TIDs) && containsAllTIDS(jUL, newgen_TIDs))) {
    ...
    /*
     * LBP strategy
     * 计算conjunction upper bound utility
     * */
...
//        return cdiff;
    ...
    /**
     * 在执行包含性检查之前执行时那个预检查条件，满足三个条件之后在做包含性检查，否则不执行包含性检查操作
     * The improved isSubsumedCheck procedure
    ...
    /**
     * The Coverage Construct Procedure
